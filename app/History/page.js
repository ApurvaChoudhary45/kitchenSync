    ...
    <div>
      <div className="min-h-screen flex flex-col">
            <div className="relative w-full flex-grow">
                <img
                    src="https://images.pexels.com/photos/9461752/pexels-photo-9461752.jpeg?auto=compress&cs=tinysrgb&w=600"
                    alt="No image to display"
                    className="w-full h-full object-cover blur-xs absolute inset-0"
                />
                <div className="relative z-10">
                   
                    <motion.div className="top-0 left-0 right-0 z-50 flex justify-around mx-5 py-4 bg-black/40 backdrop-blur-sm shadow-lg rounded-b-lg fixed" variants={contianer(0.1)} initial='initial' animate='animate'>
          ...
          return (
            <Link key={tab} href={path} onClick={() => setMobileMenuOpen(false)}>
              <span className="block px-4 py-2 rounded hover:bg-white/10">{tab}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 py-20 px-5">
                        {Array.isArray(history) && history.length > 0 && history.map(item => (
                            <motion.div className="bg-white shadow-lg rounded-xl overflow-hidden w-full md:w-3/4 mx-auto my-5" key={item.id} whileInView={{opacity:1, y:0}} initial={{opacity:0, y:100}} transition={{duration: 0.5}}>
                                <img
                                    src={item.img}
                                    alt="Dish"
                                ...
                                <div className="p-5 flex flex-col gap-2">
                                    ...
                                    <button
                                        className="mt-4 w-full bg-orange-500 text-white py-2 rounded-md hover:bg-orange-600 transition text-center cursor-pointer"
